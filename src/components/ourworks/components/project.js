...
function Header(props) {
  ...
  const { entry, index, loading } = props;
  const parse2 = (text, placeholder) =>
    loading ? placeholder ?? "" : parser(text?.["en"] ?? "");
  return (
    <div className={styles.root} key={index}>
      <img alt="ourworks" src={entry?.imageDetail?.original} className={clsx(styles.img, 'wow slideInLeft')} />
        ...
        <div className={styles.wrapField}>
          {entry?.type?.map((entryField, indexField) => (
            <div className={styles.field} key={indexField}>
              <span>{entryField?.name}</span>
            </div>
          ))}
        </div>
        <h4 className={styles.projectName}>{entry.name}</h4>
        <div className={styles.wrapListInfo}>
          {!loadingLP && !errorLP && leftProject.name.map((entryListInfo, indexListInfo) => (
            <div className={styles.row} key={indexListInfo}>
              ...
              {parse2(entryListInfo) === "Technologies" ? (
                <div className={styles.rightInfo}>
                  {entry?.development?.map((entryTechList, indexTechList) => (
                    <div className={styles.wrapTech} key={indexTechList}>
                      <img
                        alt="technology"
                        src={!loading ? entryTechList?.image?.original : undefined}
                        className={styles.iconTech}
                      />
                      <span className={styles.techName}>
                        {!loading ? entryTechList?.name : undefined}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div className={styles.rightInfo}>
                  <a href={!loading ? entry?.url : "/"} className={styles.link}>
                    {entry?.url}
                  </a>
                </div>
